...
for($i=0;$i -lt $content_length3;$i++)
{
	...
	if($2 -ne $null)
		{
			Remove-ADComputer -Identity $Computer_Name -Confirm:$False
			$1 = Get-ADComputer -identity $Computer_Name
			if($1 -ne $null)
				{
					Write-Host "AD Computer $Computer_Name Remove Failed."
				}
			else
				{
					Write-Host "AD Computer $Computer_Name Remove Successful."
				}
		}
	else
		{
